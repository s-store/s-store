...
public class TestHSQLDB extends TestCase {
    ...
    /*
     * public void testCatalogRead() { String ddl =
     * "create table test (cash integer default 23);"; HSQLInterface hsql =
     * HSQLInterface.loadHsqldb(); try { hsql.runDDLCommand(ddl); } catch
     * (HSQLInterface.HSQLParseException e1) { assertFalse(true); } String xml =
     * hsql.getXMLFromCatalog(); assertTrue(xml != null); String sql =
     * "select * from test;"; try { xml = hsql.getXMLCompiledStatement(sql); }
     * catch (HSQLInterface.HSQLParseException e) { e.printStackTrace(); }
     * //System.out.println(xml); assertTrue(xml != null); }
     */
    ...
    /*
     * public void testSimpleSQL() { HSQLInterface hsql = setupTPCCDDL();
     * assertFalse(hsql == null); String sql =
     * "SELECT O_ID, O_CARRIER_ID, O_ENTRY_D FROM ORDERS WHERE O_W_ID = ? AND O_D_ID = ? AND O_C_ID = ? ORDER BY O_ID DESC LIMIT 1;"
     * ; System.out.println(sql); String xml = null; try { xml =
     * hsql.getXMLCompiledStatement(sql); } catch (HSQLParseException e1) {
     * e1.printStackTrace(); } assertFalse(xml == null);
     * System.out.println(xml); StringInputStream xmlStream = new
     * StringInputStream(xml); Document doc = null; DocumentBuilderFactory
     * factory = DocumentBuilderFactory.newInstance();
     * factory.setValidating(true); try { DocumentBuilder builder =
     * factory.newDocumentBuilder(); builder.setErrorHandler(new
     * VoltErrorHandler()); doc = builder.parse(xmlStream); } catch (Exception
     * e) { e.printStackTrace(); //assertTrue(false); } assertFalse(doc ==
     * null); }
     */

    /*
     * public void testDeleteIssue() { HSQLInterface hsql = setupTPCCDDL();
     * assertFalse(hsql == null); //String stmt =
     * "delete from NEW_ORDER where NO_O_ID = 1;"; //String stmt =
     * "delete from NEW_ORDER where NO_O_ID = 1 and NO_D_ID = 1 and NO_W_ID = 1;"
     * ; String stmt =
     * "delete from NEW_ORDER where NO_O_ID = 1 and NO_D_ID = 1 and NO_W_ID = 1 and NO_W_ID = 3;"
     * ; String xml = null; try { xml = hsql.getXMLCompiledStatement(stmt); }
     * catch (HSQLParseException e1) { e1.printStackTrace(); } assertFalse(xml
     * == null); System.out.println(xml); }
     */
